...
int main(void)
{
  ...
  MX_TIM1_Init();
  /* USER CODE BEGIN 2 */
  void controlMotors(uint32_t dutyCycle, uint32_t delay_ms);// 모터 ?��방향 ?��?��
 {
...
/* USER CODE BEGIN 4 */
void controlMotors(uint32_t dutyCycle, uint32_t delay_ms) {
    // 1. 각 모터의 방향 및 PWM 신호 설정
    ...
    // 2. 모터 정지 (PWM duty cycle을 0으로 설정하여 모터를 정지)

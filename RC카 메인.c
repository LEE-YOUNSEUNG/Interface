...
int main(void)
{
  ...
  void lee(uint32_t dutyCycle, uint32_t delay_ms);
  void leeRight(uint32_t dutyCycle, uint32_t delay_ms);
  void leeLeft(uint32_t dutyCycle, uint32_t delay_ms);
 {
  ...
  while (1)
  {
	  HAL_UART_Receive(&huart1, (uint8_t*)rx_msg, 50, 500);
	 	  ...
	 	  else if(rx_msg[0]=='f'&&rx_msg[1]=='o'&&rx_msg[2]=='r'&&rx_msg[3]=='w'&&rx_msg[4]=='a'&&rx_msg[5]=='r'&&rx_msg[6]=='d')
	 	  {
	 	  forward(1000,1);
...
void backward(uint32_t dutyCycle, uint32_t delay_ms) {
    // 1. �???????????? 모터?�� 방향 ?��?�� �???????????? PWM 채널 ?��?��?��
	HAL_GPIO_WritePin(GPIOC, motor_1_IN1_Pin|motor_2_IN3_Pin|motor_3_IN1_Pin|motor_4_IN3_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOC, motor_1_IN2_Pin|motor_2_IN4_Pin|motor_3_IN2_Pin|motor_4_IN4_Pin, GPIO_PIN_SET);
...
void turnLeft(uint32_t initialDutyCycle, uint32_t minDutyCycle, uint32_t delay_ms) {
    // 1. 모터 1번과 3번의 방향?�� ?��?��?��?�� PWM 출력
    HAL_GPIO_WritePin(GPIOC, motor_3_IN1_Pin | motor_4_IN3_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOC, motor_3_IN2_Pin | motor_4_IN4_Pin, GPIO_PIN_RESET);
...
void turnRight(uint32_t initialDutyCycle, uint32_t minDutyCycle, uint32_t delay_ms) {
    // 1. 모터 2번과 4번의 방향?�� ?��?��?��?�� PWM 출력
    HAL_GPIO_WritePin(GPIOC, motor_1_IN1_Pin | motor_2_IN3_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOC, motor_1_IN2_Pin | motor_2_IN4_Pin, GPIO_PIN_RESET);
...
void oneLeft(uint32_t dutyCycle, uint32_t delay_ms) {

    HAL_GPIO_WritePin(GPIOC, motor_1_IN1_Pin|motor_2_IN3_Pin|motor_3_IN2_Pin|motor_4_IN4_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIOC, motor_1_IN2_Pin|motor_2_IN4_Pin|motor_3_IN1_Pin|motor_4_IN3_Pin,GPIO_PIN_RESET);
...
void oneRight(uint32_t dutyCycle, uint32_t delay_ms) {
    HAL_GPIO_WritePin(GPIOC, motor_1_IN1_Pin|motor_2_IN3_Pin|motor_3_IN2_Pin|motor_4_IN4_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIOC, motor_1_IN2_Pin|motor_2_IN4_Pin|motor_3_IN1_Pin|motor_4_IN3_Pin, GPIO_PIN_SET);
